...
async def _get_granters_prompt(actuator_name: str) -> str:
    ...
    granters: List[User] = await md.get_mediator().actuators.get_granters(actuator_name)
    if granters:
...
async def _get_actuators_prompt():
    ...
    actuators: List[Actuator] = await md.get_mediator().actuators.get_all()
    if actuators:
...
async def _get_users_prompt() -> str:
    ...
    users: List[User] = await md.get_mediator().users.get_all()
    if users:
...
async def _get_subscribers_prompt(channel: str) -> str:
    ...
    subscribers = await md.get_mediator().channels.get_subscribers(channel)
