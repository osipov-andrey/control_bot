...
class SubscribeHandler(MessageHandler):
    ...
    async def handle(self, message: types.Message, state: FSMContext, **kwargs):
        cmd = await get_subscribe_or_unsubscribe_cmd(
            SUBSCRIBE, self.user_telegram_id, self.is_admin
        )
        ...
        async def subscribe_callback(**kwargs_):
            ...
            try:
                ...
                await message.answer(
                    f"The user {user_to_subs_id} is ALREADY subscribed to the channel {channel}"
                )
            else:
                await message.answer(
                    f"The user {user_to_subs_id} is subscribed to the channel {channel}"
                )
...
class UnsubscribeHandler(MessageHandler):
    ...
    async def handle(self, message: types.Message, state: FSMContext, **kwargs):
        cmd = await get_subscribe_or_unsubscribe_cmd(
            UNSUBSCRIBE, self.user_telegram_id, self.is_admin
        )
